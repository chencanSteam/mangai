(function () {
  ...
  function renderUserAuth() {
    ...
    return `<div class="user-auth-screen"><section class="user-auth-card"><div class="user-auth-kicker">MAN GAI Mock</div><h2>用户登录</h2>${state.userAuthFeedback ? `<div class="provider-feedback">${state.userAuthFeedback}</div>` : ""}<form class="provider-complete-form user-auth-form" data-user-auth-form data-auth-mode="login"><div class="form-grid"><div class="field-group"><label class="field-label" for="auth-phone">手机号</label><input class="input" id="auth-phone" name="phone" type="tel" value="13800138000" required></div><div class="field-group"><label class="field-label" for="auth-password">密码</label><input class="input" id="auth-password" name="password" type="password" value="mock1234" required></div></div><div class="admin-action-row"><button class="btn btn-primary" type="submit">登录</button></div></form><div style="text-align:center; margin-top:18px;"><div style="font-size:12px; color:var(--text-muted); margin-bottom:10px;">其他登录方式</div><button type="button" data-user-auth-mode="registerPhone" style="width:44px; height:44px; border-radius:50%; background:#07c160; color:#fff; border:none; font-size:20px; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; box-shadow:0 4px 12px rgba(7,193,96,0.25);">💬</button></div><div style="text-align:center; margin-top:16px;"><button type="button" data-user-auth-mode="registerPhone" style="background:none; border:none; color:var(--text-muted); font-size:13px; cursor:pointer;">还没有账号？去注册</button></div></section></div>`;
  }
