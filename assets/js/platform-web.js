(function () {
  ...
  function openProductEditorModal(mode, row) {
    ...
    const syncProductPreview = () => {
      ...
      const specText = selectedSpecs.filter((spec) => spec.name).map((spec) => `${spec.name}：${(spec.options || []).slice(0, 6).join(" / ") || "待填写"}`);
      preview.innerHTML = `<div class="product-preview-label">实时商品预览</div><div class="product-preview-image">${image ? `<img src="${escapeAttribute(image)}" alt="商品主图" />` : `<span>商品主图</span>`}</div><div class="product-preview-brand">${escapeHtml(getField("brand") || "品牌未填写")} · ${escapeHtml(getField("category") || "未选择类目")}</div><h3>${escapeHtml(getField("name") || "商品名称待填写")}</h3><div class="product-preview-price">${priceText}</div><div class="product-preview-fitment">${fitment ? escapeHtml(fitment) : "暂未选择适配车型"}</div><div class="product-preview-stats"><span>${rows.length} 个 SKU</span><span>库存 ${rows.reduce((sum, item) => sum + (Number(item.stock) || 0), 0)}</span><span>${sellable.length ? "有库存" : "暂时缺货"}</span></div>${specText.length ? `<div class="product-preview-specs">${specText.map((text) => `<div class="product-preview-spec-row"><b>${escapeHtml(text.split("：")[0])}</b><span>${escapeHtml(text.split("：").slice(1).join("："))}</span></div>`).join("")}</div>` : ""}${getField("promoLabel") ? `<div class="product-preview-promo">${escapeHtml(getField("promoLabel"))} · ${escapeHtml(getField("promoDiscount"))}</div>` : ""}<div class="product-preview-detail pd-rich-content">${detail || `<p class="muted">商品详情将在这里展示</p>`}</div>`;
    };
