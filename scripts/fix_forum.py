...
with open('assets/js/mobile-app.js', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find all renderUserForum() function starts (not Form/Detail/ReplyForm)
starts = []
for i, line in enumerate(lines):
    if 'function renderUserForum()' in line and 'Form' not in line and 'Detail' not in line and 'Reply' not in line:
        starts.append(i)

print(f"Found {len(starts)} renderUserForum() definitions at lines: {[s+1 for s in starts]}")
...
print(f"Function ends at lines: {[e+1 for e in ends]}")

# New renderUserForum function
new_func = '''  function renderUserForum() {
    const forumCards = [
...
# Build new content: keep before first, add new func, keep after last
new_lines = lines[:starts[0]]
new_lines.append(new_func)
new_lines.extend(lines[ends[-1]+1:])

with open('assets/js/mobile-app.js', 'w', encoding='utf-8') as f:
